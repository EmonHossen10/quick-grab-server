...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@cluster0.r0kjzd3.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  try {
    ...
    // here all apis coming ---------->>>>
    const userCollection = client.db("quickDb").collection("users");
    const menuCollection = client.db("quickDb").collection("menu");
    ...
    app.post("/users", async (req, res) => {
      const user = req.body;
      // check user existing or not in the database
...
app.get("/", (req, res) => {
  res.send(
    `This is Our server which shows on localhost is running on   ${port}`
  );
});

app.listen(port, () => {
  console.log(`Quick grab server is running on ${port}`);
});
